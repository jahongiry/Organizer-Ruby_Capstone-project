...
module MusicAlbumController
  def save_music_album(album)
    file.new('./controllers//music_albums.json', + 'w+') unless File.exist?('./controllers/music_albums.json')

    if File.empty?('./controllers/music_albums.json')
      music_albums = []
    else
      data = File.read('./controllers/music_albums.json').split
    ...
    File.write('./controller/music_albums.json', music_albums.to_json)
  end
